...
class SimpleFileLoader {
     ...
     *                                    test case classes.
     *    @return GroupTest               The new test suite.
     *    @access public
     */
    ...
    function selectRunnableTests($candidates) {
        ...
        foreach ($candidates as $class) {
            if (GroupTest::getBaseTestCase($class)) {
    ...
    function createSuiteFromClasses($title, $classes) {
        if (count($classes) == 0) {
            $suite = new BadGroupTest($title, "No runnable test cases in [$title]");
            return $suite;
        }
        SimpleTest::ignoreParentsIfIgnored($classes);
        $suite = new GroupTest($title);
...
class GroupTest {
    ...
    function add($test_case) {
        ...
        } elseif (GroupTest::getBaseTestCase($test_case) == 'GroupTest') {
